...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($text === '' || mb_strlen($text) > 5000) {
        ...
        $apiUrl = rtrim((string) getenv('API_URL'), '/');
        $username = (string) getenv('API_USERNAME');
        $password = (string) getenv('API_PASSWORD');
        if ($apiUrl === '' || $username === '' || $password === '') {
            $error = 'The client is not configured.';
        } else {
            $handle = curl_init($apiUrl . '/api/translate');
            ...
            if ($status === 200 && is_array($payload) && isset($payload['translation'])) {
                ...
                $error = is_array($payload) && isset($payload['error'])
                    ? (string) $payload['error']
                    : ($curlError !== '' ? 'Could not reach the server.' : 'Translation failed.');
            }
        }
    }
}
